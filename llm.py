...
def _ozon_clothing_title_instruction(category_profile: dict[str, Any] | None) -> str:
    ...
    if not any(marker in category for marker in OZON_CLOTHING_CATEGORY_MARKERS):
        return ""
    return (
        " Для одежды название должно быть содержательным, а не состоять только из типа товара и цвета."
        " Формула названия для одежды: тип товара + бренд/надпись/модель при наличии + цвет + 1-2 подтвержденные отличительные особенности."
        " В название можно вынести подтвержденные особенности кроя и конструкции: облегающий, оверсайз, с воротником-стойкой, с принтом на спине, на молнии."
